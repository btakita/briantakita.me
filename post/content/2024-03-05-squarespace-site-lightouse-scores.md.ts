...
Google & other search engines ${tb_a_({ href: 'https://developers.google.com/search/blog/2010/04/using-site-speed-in-web-search-ranking', nofollow: true }, 'factor page load time')} in ranking search results. Page speed ensures the person has an optimal experience viewing the search result.

## The Lighthouse Test Tool

Lighthouse is ${tb_a_({ href: 'https://developer.chrome.com/docs/lighthouse/overview/', nofollow: true }, 'built into Google Chrome')}. To use:
...
## Case 1 - ${tb_a_({ href: 'https://www.artipsstudio.com/', nofollow: true }, 'www.artipsstudio.com')}
...
## Case 2 - ${tb_a_({ href: 'https://www.jonesbbqkc.com/', nofollow: true }, 'www.jonesbbqkc.com')}
...
## Case 3 - ${tb_a_({ href: 'https://devoe-fluid-demo.squarespace.com/' }, 'devoe-fluid-demo.squarespace.com')}

${tb_a_({ href: 'https://www.stylefactoryproductions.com/blog/how-to-speed-up-a-squarespace-site', nofollow: true }, 'How to Speed Up a Squarespace Site')} features the Devoe template. For it's performance improvements over other Squarespace Site templates. With a performance score of 55, Squarespace has a ways to go.
...
Alternative e-commerce platforms are another option. Though Squarespace does score well compared to it's direct competitors, such as Shopify. The ${tb_a_({ href: 'https://boostertheme.com/' }, 'Booster theme')} is the fastest Shopify theme I found for this article.

## Bonus Shopify Booter Theme Case Study - ${tb_a_({ href: 'https://dodropshipping.com/', nofollow: true })}

dodropshipping.com has a performance score of 85. A decent score! My hunch is that Shopify has an advantage due it's 3rd party theme ecosystem. These 3rd party themes boutique developers to specialize. The Booster theme team delivers high performance Shopify themes.

${lighthouse__iframe_(dodropshipping_html)}

## Why are there no e-commerce themes with good (>90) to Perfect Performance Scores?
