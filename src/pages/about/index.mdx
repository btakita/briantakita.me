...
active: about
---
import { atb_, footnote_list_c_ } from '@btakita/ui--server--blog'
import {
	atb_astrojs_,
	atb_bunjs_,
	atb_ctx_core_,
	atb_electric_universe_,
	atb_existence_,
...
<R $={p_(
	...
	atb_({ href: '/posts/magic-connections-with-the-philosophy-of-language'}, `language`), ` to model any reified
	entity. Language patterns can be utilized in software & speech to create models of Existence.`
)}/>

<R $={h2_(atb_electric_universe_())}/>
...
<R $={p_(
	`Agent-centric crypto platform, inspired by nature. The `, atb_holochain_whitepaper_(), ` is worth a read. Here
	are some `, atb_({ href: 'https://github.com/btakita/holochain-notes/blob/master/whitepaper.md' }, 'notes'), `.`
)}/>
