...
import { Atb, V_atb, V_footnote_list } from '@btakita/ui--server--blog'
import {
	V_atb_astrojs,
	V_atb_bunjs,
	V_atb_ctx_core,
...
export const H = H_(ctx)

I create full stack software solutions. My current preferred tooling is:
<Van dom={
	H.ul(
		H.li('monorepos'),
		H.li(V_atb_ctx_core({ ctx })),
		H.li(V_atb_bunjs({ ctx })),
		H.li(V_atb_astrojs({ ctx })),
		H.li(V_atb_nanostores({ ctx })),
		H.li(V_atb_solidjs({ ctx })),
		H.li(V_atb_redis({ ctx })),
		H.li(V_atb_postgres({ ctx }))
	)
}/>
...
<p>
	{[
		...
		<Van dom={V_atb_existence({ ctx })}/>,
		`?" sprang a `,
		<Van dom={V_atb_philosophy({ ctx })}/>,
		`.`
	]}
</p>
<p>
...
<h2>
	<Van dom={V_atb_electric_universe({ ctx })}/>
</h2>
...
<p>
	{[
		...
		<Van dom={V_atb_holochain_whitepaper({ ctx })}/>,
		` is worth a read. Here are some `,
		<Van dom={
			V_atb({ ctx, href: 'https://github.com/btakita/holochain-notes/blob/master/whitepaper.md' },
				'notes')
		}/>,
		`.`
	]}
...
<div>
	...
	<Van dom={V_attribution_dev({ ctx })}/>
</div>

<Van dom={V_footnote_list({ ctx })}/>
