...
active: about
---
import { tb_a_, footnote_list_c_ } from '@btakita/ui--server--blog'
import {
	astrojs__tb_a_,
	bunjs__tb_a_,
	ctx_core__tb_a_,
	...
	redis__tb_a_,
	solidjs__tb_a_,
	attribution_dev__add,
} from '@btakita/ui--server--briantakita'
import R from 'relementjs/astro'
import { div_, h2_, img_, li_, p_, ul_ } from 'relementjs/html'
...
<R $={p_(
	...
	tb_a_({ href: '/posts/magic-connections-with-the-philosophy-of-language'}, `language`), ` to model any reified
	entity. Language patterns can be utilized in software & speech to create models of Existence.`
)}/>

<R $={h2_(electric_universe__tb_a_())}/>
...
<R $={p_(
	`Agent-centric crypto platform, inspired by nature. The `, holochain_whitepaper__tb_a_(), ` is worth a read. Here
	are some `, tb_a_({ href: 'https://github.com/btakita/holochain-notes/blob/master/whitepaper.md' }, 'notes'), `.`
)}/>
...
<R $={div_(
	img_({ src: '/assets/images/dev.svg', class: 'sm:w-1/2 mx-auto', alt: 'coding dev illustration' }),
	attribution_dev__add({ ctx })
)}/>
