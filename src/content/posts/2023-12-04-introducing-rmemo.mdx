...
<R $={[p_(
	`I'm pleased to announce `, rmemo__tb_a_(), `, a tiny no-fluff reactive state library. rmemo is the reactive core
	of `, relementjs__tb_a_(), `, a server & browser UI rendering library that I also released.`
)]}/>
...
<R $={p_(
	`rmemo tracks listeners using `, WeakRef__tb_a_(), `. Most other reactive state management libraries directly track
...
<R $={p_(
	`Until recently I was using `, nanostores__tb_a_(), ` for my goto state management solution. I contributed to fixing
	diamond dependency issues. I also wrote some extension libraries`,
	footnote_c_({ ctx, id: 'ctx-core-nanostore' }, ctx_core_nanostores__tb_a_(), ` & `, ctx_core_solid_nanostores__tb_a_()),
