...
import { pugjs__tb_a_, sveltejs__tb_a_ } from '@btakita/ui--server--briantakita'
...
<R $={p_(
	`On my current project, I will be migrating from `, pugjs__tb_a_(), ` to `, sveltejs__tb_a_(), `. The key concepts are
	`, tb_a_({ href: 'https://svelte.technology/guide#server-side-rendering' }, 'Server Side Rendering'), `, `,
	tb_a_({ href: 'https://github.com/sveltejs/svelte/pull/649' }, 'Hydration'), `, client/server architectural options.`
)}/>
