...
<R $={p_(
	...
	code_(`"Hello " + user.name`), ` and one for `, code(`"Nice to see you"`), `. The callback is invoked when the
	value is`, atb_({ innerText: 'Truthy', href: 'http://www.sitepoint.com/javascript-truthy-falsy/' }), `. So when `,
