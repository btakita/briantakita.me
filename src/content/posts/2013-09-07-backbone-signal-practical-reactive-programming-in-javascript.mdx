...
import { backbone_signal__tb_a_, browserify__tb_a_, nodejs__tb_a_, rundavoo__tb_a_ } from '@btakita/ui--server--briantakita'
import R from 'relementjs/astro'
import { code_, p_ } from 'relementjs/html'

<R $={p_(
	`I have recently released `, backbone_signal__tb_a_(), `, which is a `,
	tb_a_({
...
<R $={p_(
	`backbone-signal is being used in `, rundavoo__tb_a_(), ` and has been fun to use, especially with `, nodejs__tb_a_(),
	` & `, browserify__tb_a_(), `. It's been a pleasure using a lightweight unframework to freely structure the dataflow
	logic of the site.`
)}/>
