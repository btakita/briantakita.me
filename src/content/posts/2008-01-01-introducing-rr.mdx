...
<p>
	<Rel dom={
		fragment_(
			`I'm pleased to introduce a new Test Double (or mock) framework named `,
			atb_rr_(),
			`, which is short for Double Ruby. Why a Double framework and not a Mock framework?`)
	}/>
</p>
...
<p>
	<Rel dom={
		fragment_(
			`Unfortunately, the terminology over doubles has been contradictory depending on the framework. RR's
			terminology tries to be as faithful as possible to Gerald Meszaros' definition of test doubles. You can read
			more about test doubles in `,
			atb_({ href: 'http://xunitpatterns.com/Test%20Double%20Patterns.html' }),
			` and Martin Fowler's article, `,
			atb_({ href: 'http://martinfowler.com/articles/mocksArentStubs.html' }),
...
<p>
	<Rel dom={
		fragment_(
			`I'll use trivial Rails examples to highlight the syntactical differences between RR, `,
			atb_mocharb_(),
			`, `,
			atb_rspec_mocks_(),
			`, `,
			atb_flexmockrb_(),
			`. They may or may not be appropriate situations for mocks. The right situations for mocks is an entirely
