...
import { V_atb_flexmockrb, V_atb_mocharb, V_atb_rr, V_atb_rspec_mocks } from '@btakita/ui--server--briantakita'
...
<p>
	<Van dom={
		V_fragment(props,
			...
			V_atb({ ctx: props.ctx, href: 'http://xunitpatterns.com/Test2e.html' }),
			` and Martin Fowler's article, `,
			V_atb({ ctx: props.ctx, href: 'http://martinfowler.com/articles/mocksArentStubs.html' }),
...
<p>
	<Van dom={
		V_fragment(props,
			...
			V_atb_mocharb(props),
			`, `,
			V_atb_rspec_mocks(props),
			`, `,
			V_atb_flexmockrb(props),
