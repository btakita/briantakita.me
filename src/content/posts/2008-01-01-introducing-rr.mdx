...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			V_atb({ ctx, href: 'http://xunitpatterns.com/Test%20Double%20Patterns.html' }),
			` and Martin Fowler's article, `,
			V_atb({ ctx, href: 'http://martinfowler.com/articles/mocksArentStubs.html' }),
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			V_atb_mocharb({ ctx }),
			`, `,
			V_atb_rspec_mocks({ ctx }),
			`, `,
			V_atb_flexmockrb({ ctx }),
