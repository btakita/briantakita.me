...
export const { code, li, ul } = H_(ctx)
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			V_footnote({ ctx, id: 'early-freelance-projects' },
			...
			V_atb_linux({ ctx }),
			` & exploring `,
			V_atb_literate_programming({ ctx }),
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			V_atb({ ctx, innerText: 'CRM', href: 'https://en.wikipedia.org/wiki/Customer_relationship_management' }),
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			V_atb_grockit({ ctx }),
			`. `,
			V_atb({ ctx, innerText: 'Reactive', href: 'https://en.wikipedia.org/wiki/Reactive_programming' }),
			` `,
			V_atb({ ctx, innerText: 'state management', href: 'https://en.wikipedia.org/wiki/State_management' }),
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			<code>describe</code>,
			` statements`,
			V_footnote({ ctx, id: 'nested-describe-statements' },
				...
				V_atb({ ctx, innerText: 'Tag Vectors', href: '/posts/tag-vector-0-introduction' }),
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			V_atb_monorepo({ ctx, innerText: 'monorepo' }),
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			journey, my `,
			V_atb_worldview({ ctx, innerText: 'worldview' }),
			` expanded with experimenting with the `,
			V_atb_ontology({ ctx, innerText: 'ontological' }),
			` expression of a `,
			V_atb_philosophy({ ctx }),
			`. During this exploration, I explored how word `,
			V_atb_definition({ ctx, innerText: 'definitions' }),
			` can be `,
			V_atb_scope({ ctx, innerText: 'scoped' }),
			` with `,
			V_atb_context({ ctx, innerText: 'context' }),
			V_footnote({ ctx, id: 'context-of-entity' },
				`Taking into account that what is spoken & what is heard is defined with the context of the `,
				V_atb_entity({ ctx, innerText: 'entity' }),
				` speaking or hearing.`
			),
			`. I learned about `,
			V_atb_existence({ ctx }),
			` and in a wierd way, `,
			V_atb({ ctx, innerText: 'Everything Exists', href: '/posts/everything-exists' }),
			`.`)
	}/>
</p>
<h2>
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			V_footnote({ ctx, id: 'tag-vector-development-continues' },
...
<Van dom={
	ul(
		...
		li(`to `, V_atb_solidjs({ ctx }), ` with `, V_atb_nanostores({ ctx }), ` & `, V_atb_solid_start({ ctx })),
		li(`to solidjs with Nano Stores & `, V_atb_astrojs({ ctx })))
}/>
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			V_footnote({ ctx, id: 'vanjs-tradeoff' },
...
<p>
	<Van dom={
		V_fragment({ ctx },
			`Astrojs provides `,
			V_atb({ ctx, href: 'https://docs.astro.build/en/guides/client-side-scripts/#client-side-scripts' },
				`client side builds`),
			`, an independent browser build with the code inside of the `,
			code(`<script/>`),
			` tag. The Astrojs team pointed out techniques such as `,
			V_atb({ ctx, href: 'https://docs.astro.build/en/reference/directives-reference/#definevars' },
				`passing variables`),
			` into the `,
			code(`<script/>`),
			` tags & `,
...
<p>
	<Van dom={
		V_fragment({ ctx },
			...
			build tools, `,
			V_atb({ ctx, href: 'https://cleancommit.io/blog/spa-vs-mpa-which-is-the-king/' },
				`MPAs vs SPAs`),
			`, `,
			V_atb({ ctx, href: 'https://jamstack.org/' },
				`Jamstack`),
			`, `,
			V_atb({ ctx, href: 'https://dev.to/this-is-learning/understanding-transitional-javascript-apps-27i2'},
				`Transitional Apps`),
