...
<R $={p_(
	...
	`. I learned resiliency in uncertain times where money was scarce & using my free time to work on the craft;
		...
		early influence as it promotes software code embedded in prose & formatted text. I continue my work on tooling to support Literate programming with low-code solutions. I was
		fascinated with this programming language from Japan called `, atb_ruby_(), ` & grew even more excited with this
		cool alpha web framework called `, atb_ruby_on_rails_(), `.`
)}/>
<R $={p_(
	`I got my first full time job at `,
	atb_naspp_(), `. The job consisted of office automation software & maintaining a long-used `,
	atb_({ innerText: 'CRM', href: 'https://en.wikipedia.org/wiki/Customer_relationship_management' }),
...
<R $={p_(
	...
	atb_({ innerText: 'Reactive', href: 'https://en.wikipedia.org/wiki/Reactive_programming' }), ` `,
	atb_({ innerText: 'state management', href: 'https://en.wikipedia.org/wiki/State_management' }), ` clicked for
		me, as the paradigm allows state to be composed of functions with reactive inputs & a single output.`
)}/>
<R $={p_(
	`I was a core contributor to the Behavior Driven Development library `, atb_rspec_(), ` & championed/developed
			the first (that I know of) implementation of nested `, code_('describe'), ` statements`,
	footnote_c_({ ctx, id: 'nested-describe-statements' },
		...
		atb_({ innerText: 'Tag Vectors', href: '/posts/tag-vector-0-introduction' }),
...
<R $={p_(
	...
	atb_monorepo_({ innerText: 'monorepo' }),
...
<R $={p_(
	`Out of paying work, I went on a personal journey of sorts, visiting up & down the West Coast. During this
			journey, my `,
	atb_worldview_({ innerText: 'worldview' }),
	` expanded with experimenting with the `,
	atb_ontology_({ innerText: 'ontological' }),
	` expression of a `,
	atb_philosophy_(),
	`. During this exploration, I explored how word `,
	atb_definition_({ innerText: 'definitions' }),
	` can be `,
	atb_scope_({ innerText: 'scoped' }),
	` with `,
	atb_context_({ innerText: 'context' }),
	footnote_c_({ ctx, id: 'context-of-entity' },
		...
		atb_entity_({ innerText: 'entity' }),
		` speaking or hearing.`
	),
	`. I learned about `,
	atb_existence_(),
	` and in a wierd way, `,
	atb_({ innerText: 'Everything Exists', href: '/posts/everything-exists' }),
	`.`
)}/>
<h2>
	<R $={atb_neo_()}/>
...
<R $={p_(
	...
	atb_menu_(),
	` & `,
	atb_({ innerText: 'an app', href: 'https://github.com/btakita/iron-henhouse' }),
...
<R $={p_(
	...
	atb_({ href: 'https://dev.to/joetex/react-why-so-complicated-3dc7' },
		'complicated API'),
...
<R $={p_(
	`Astrojs provides `,
	atb_({ href: 'https://docs.astro.build/en/guides/client-side-scripts/#client-side-scripts' },
		`client side builds`),
	`, an independent browser build with the code inside of the `,
	code_(`<script/>`),
	` tag. The Astrojs team pointed out techniques such as `,
	atb_({ href: 'https://docs.astro.build/en/reference/directives-reference/#definevars' },
		`passing variables`),
	` into the `,
	code_(`<script/>`),
	` tags & `,
...
<R $={p_(
	...
	code_(`cgi->server markup languages->web frameworks`),
	`. Large javascript payloads, SEO, chatty apis, caching, hydration, etc. With Astrojs, the trade-off between
			build tools, `,
	atb_({ href: 'https://cleancommit.io/blog/spa-vs-mpa-which-is-the-king/' },
		`MPAs vs SPAs`),
	`, `,
	atb_({ href: 'https://jamstack.org/' },
		`Jamstack`),
	`, `,
	atb_({ href: 'https://dev.to/this-is-learning/understanding-transitional-javascript-apps-27i2'},
		`Transitional Apps`),
