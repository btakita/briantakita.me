...
<p>
	<Rel dom={
		fragment_(
			...
			V_atb({ innerText: 'CRM', href: 'https://en.wikipedia.org/wiki/Customer_relationship_management' }),
...
<p>
	<Rel dom={
		fragment_(
			...
			V_atb_grockit(),
			`. `,
			V_atb({ innerText: 'Reactive', href: 'https://en.wikipedia.org/wiki/Reactive_programming' }),
			` `,
			V_atb({ innerText: 'state management', href: 'https://en.wikipedia.org/wiki/State_management' }),
	...
	<Rel dom={
		fragment_(
			...
			V_footnote({ ctx, id: 'nested-describe-statements' },
				...
				V_atb({ innerText: 'Tag Vectors', href: '/posts/tag-vector-0-introduction' }),
...
<p>
	<Rel dom={
		fragment_(
			...
			V_atb_monorepo({ innerText: 'monorepo' }),
...
<p>
	<Rel dom={
		fragment_(
			...
			journey, my `,
			V_atb_worldview({ innerText: 'worldview' }),
			` expanded with experimenting with the `,
			V_atb_ontology({ innerText: 'ontological' }),
			` expression of a `,
			V_atb_philosophy(),
			`. During this exploration, I explored how word `,
			V_atb_definition({ innerText: 'definitions' }),
			` can be `,
			V_atb_scope({ innerText: 'scoped' }),
			` with `,
			V_atb_context({ innerText: 'context' }),
			V_footnote({ ctx, id: 'context-of-entity' },
				...
				V_atb_entity({ innerText: 'entity' }),
				` speaking or hearing.`
			),
			`. I learned about `,
			V_atb_existence(),
			` and in a wierd way, `,
			V_atb({ innerText: 'Everything Exists', href: '/posts/everything-exists' }),
			`.`)
	}/>
</p>
<h2>
...
<p>
	<Rel dom={
		fragment_(
			...
			V_atb_menu(),
			` & `,
			V_atb({ innerText: 'an app', href: 'https://github.com/btakita/iron-henhouse' }),
...
<Rel dom={
	ul_((
		...
		li_(`to `, V_atb_solidjs(), ` with `, V_atb_nanostores(), ` & `, V_atb_solid_start()),
		li_(`to solidjs with Nano Stores & `, V_atb_astrojs())))
}/>
...
<p>
	<Rel dom={
		fragment_(
			V_atb({ href: 'https://dev.to/this-is-learning/how-react-isn-t-reactive-and-why-you-shouldn-t-care-152m' },
			...
			V_atb({ href: 'https://dev.to/joetex/react-why-so-complicated-3dc7' },
			 'complicated API'),
...
<p>
	<Rel dom={
		fragment_(
			`Astrojs provides `,
			V_atb({ href: 'https://docs.astro.build/en/guides/client-side-scripts/#client-side-scripts' },
				`client side builds`),
			`, an independent browser build with the code inside of the `,
			code_(`<script/>`),
			` tag. The Astrojs team pointed out techniques such as `,
			V_atb({ href: 'https://docs.astro.build/en/reference/directives-reference/#definevars' },
				`passing variables`),
			` into the `,
			code_(`<script/>`),
			` tags & `,
...
<p>
	<Rel dom={
		fragment_(
			...
			build tools, `,
			V_atb({ href: 'https://cleancommit.io/blog/spa-vs-mpa-which-is-the-king/' },
				`MPAs vs SPAs`),
			`, `,
			V_atb({ href: 'https://jamstack.org/' },
				`Jamstack`),
			`, `,
			V_atb({ href: 'https://dev.to/this-is-learning/understanding-transitional-javascript-apps-27i2'},
				`Transitional Apps`),
