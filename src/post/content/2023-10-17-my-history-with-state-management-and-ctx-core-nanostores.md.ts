...
${tb_a_({ href: 'https://dev.to/this-is-learning/how-react-isn-t-reactive-and-why-you-shouldn-t-care-152m' }, `React is not Reactive`)} by default. There are libraries that can add reactivity to React, like NanoStores & ${signals_react__tb_a_()}. React works with the virtual dom where I would rather work with the real DOM. Along with an ${tb_a_({ href: 'https://dev.to/joetex/react-why-so-complicated-3dc7' }, 'complicated API')} & a need for many dependencies to extend it's default behavior, I never got into React. I still cannot force myself to use React. It's not a natural fit to my mental model of developing web apps. I have grown to appreciate	contributions made by the React ecosystem.
